...
<template>
  <div id="app">
    <tracking-form @add:exercise="exercises.date" />
    <tracking-history :exercises="exercises" />
  </div>
</template>
...
export default {
  ...
  methods: {
    addExercises(exercise) {
      const addId = this.exercises.length > 0 ? this.exercises[this.exercises.length - 1].id : 0
